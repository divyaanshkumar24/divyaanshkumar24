...
| Project | What it does |
|---|---|
| **[Event-Driven Backtesting Engine](https://github.com/divyaanshkumar24/event-driven-backtesting-engine)** | A backtesting engine where look-ahead bias is structurally impossible, not just avoided by convention — the only data-access path a strategy gets is `as_of(t)`, bound to a replay clock that raises if you ask it for the future. Walk-forward optimization with purge/embargo, and every result ships with a bias audit: Deflated Sharpe Ratio, Probability of Backtest Overfitting via CSCV, gross-vs-net cost drag, capacity. A Next.js static viewer renders the report with zero Python imports. |
